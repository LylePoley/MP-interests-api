...
from backend.client.fetch import fetch_all


members_data = json.loads(
    Path("backend/client/mock_responses/mock_member_response.json").read_text()
)
interests_data = json.loads(
    Path("backend/client/mock_responses/mock_interest_response.json").read_text()
)
